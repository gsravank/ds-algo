...
def get_operations(orig_str, curr_idx):
    """
    Returns:
...
while total_cost < x and num_operations < 10**5 and operation_possible:# and (curr_time - start_time <= 4.5):
    current_operations, curr_cost, curr_idx, new_str = get_operations(orig_str, curr_idx)
...
#     items = op.strip().split(' ')
#     print(items)
#     if items[0] == '1':
#         orig_str = op_one(orig_str, int(items[1]) - 1, items[2])
#     elif items[0] == '2':
#         orig_str = op_two(orig_str, int(items[1]) - 1, int(items[2]) - 1, int(items[3]) - 1)
#     else:
#         orig_str = op_three(orig_str, int(items[1]) - 1, int(items[2]) - 1)
#
# print("String after operations: '{}'".format(orig_str))